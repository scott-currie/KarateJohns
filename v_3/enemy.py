...
AGGRESSION = .90		#Represents threshold above which random roll means to attack
KICK_THRESHOLD = .7 	#Float above which enemy decides to kick. Roll below this means punch.
	...
	def update(self, player):
		...
		#Do this stuff if not reeling
			...
			if not self.punch and not self.kick:
				'''Enemy not reeling or attacking. First check positioning, then decide whether to attack '''
				...
				elif self.hitbox.left < player.hitbox.left and self.hitbox.right > player.hitbox.left and (self.hitbox.right - player.hitbox.left >= MOVE_SPEED):
					self.walk = True
					self.rect.x -= MOVE_SPEED
				...
				if self.idle:
					'''If we've reached this point and enemy is idling, enemy is free to attack '''					
					#Should we should try to punch or kick the player.
					#First, are we colliding with player hitbox?
					if self.hitbox.colliderect(player.hitbox):
						#Roll random float. If roll higher than aggression threshold, throw a punch
						if random.random() > AGGRESSION:
							if random.random() > KICK_THRESHOLD:
								self.kick = True
							else:
								self.punch = True
							self.idle = False
							...
							#So we've thrown a punch. Let the player know he's been hit.
							
							#Only reel player if he's not already reeling or down. Don't need to worry about
							#registering multiple hits per attack, since this code will only be reached if enemy
							#started this update while not attacking; therefore, it will only be executed once.
							if not player.reel or player.down:
								player.reel = True
			...
			#by centering the hitbox center tuple using a single assignment. Keep in mind that character movement
		...
		#--Select image to display this update according to state and corresponding animation timer
